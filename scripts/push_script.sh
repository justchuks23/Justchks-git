...
sshpass -p $SERVER_PASSWORD scp -r -o StrictHostKeyChecking=no zoom_youtube_integration.tar Dockerfile docker-compose.yml scripts/deploy_script.sh entrypoint.sh $SERVER_USER@$SERVER_IP:~/
